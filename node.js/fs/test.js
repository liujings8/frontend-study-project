 ...
 * 和所有其它Javascript模块不同的是，fs模块同时提供异步和同步的方法。
 */
/**
 * 回顾以下什么是异步方法。因为JavaScript的单线程模型，执行IO操作时，
 * JavaScript代码无需等待，而是传入回调函数后，继续执行后续JavaScript代码。
...
var data = getJSON('http://exmaple.com/ajax');
/**
 ...
 * err参数为null，data参数为读取的String。
 * 当读取发生错误时，err参数代表一个错误对象，data为undefined。
 * 这也是Node.js标准的回调函数：第一个参数表示错误信息，
 * 第二个参数代表结果。后面我们还会经常编写这种回调函数。
 */
 ...
 * 在Node.js对象中，Buffer对象就是一个包含零个或任意个字节的数组(注意和Array不同)。
 * Buffer对象可以和String转换，例如，把一个Buffer对象转换成String：
 */
//Buffer -> String
 ...
 */
/**
 * 除了标准的异步读取文件外，fs也提供相应的同步读取函数。
 * 同步读取的函数和异步函数相比，多了一个Sync的后缀，并且不接收回调函数，函数直接返回结果。
 */
 ...
 * 所以绝大部分需要在服务器运行期间反复执行业务逻辑的代码，必须使用异步代码，
